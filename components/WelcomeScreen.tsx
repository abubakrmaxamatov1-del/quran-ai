...
export default function WelcomeScreen({ onGoogleEnter, onTelegramEnter, loadingProvider = null, isTelegramApp = false }: WelcomeScreenProps) {
  const motivationalQuotes = [
    "Qur'on – qalblar shifosi. Ilm olish yo'lidagi qadamingiz muborak bo'lsin!",
    "Har bir harf uchun ajr bor. O'qishni hoziroq boshlang.",
    "Qur'on o'qing, zero u Qiyomat kuni o'z sohiblariga shafoatchi bo'lib keladi."
  ];

  // Pick a random quote (stable for render if client side, but we can just use the first or pick randomly)
  // To avoid hydration mismatch, we'll just use a fixed one or a simple effect, but fixed is easier:
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-y-auto bg-gradient-to-b from-[#f0fdf4] via-white to-[#ecfdf5]">
      ...
      <div className="relative z-10 mx-auto flex min-h-[100dvh] w-full max-w-md flex-col items-center px-6 py-6">
        ...
        {/* Main Section */}
        <main className="flex w-full flex-col items-center gap-4 py-4 mt-8">
          {/* Logo with enhanced styling */}
          <div className="relative flex items-center justify-center">
            {/* Outer glow ring */}
            <div className={`absolute h-44 w-44 rounded-full bg-gradient-to-br from-emerald-200/50 to-green-100/30 blur-2xl ${isTelegramApp ? 'animate-pulse' : ''}`} />
            {/* Inner subtle ring */}
            <div className={`absolute h-36 w-36 rounded-full border border-emerald-200/50 ${isTelegramApp ? 'animate-[spin_10s_linear_infinite]' : ''}`} />
          ...
          <div className="space-y-4 text-center mt-6">
            ...
            {isTelegramApp ? (
              <div className="mx-auto max-w-xs space-y-4">
                <p className="text-sm leading-relaxed text-emerald-700/80 italic font-medium">
                  "{quote}"
                </p>
        ...
        {/* Footer with buttons - ONLY SHOW IF NOT IN TELEGRAM APP */}
        {!isTelegramApp && (
